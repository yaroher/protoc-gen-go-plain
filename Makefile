...
build-test-nda: build .clean-test-nda
	...
	LOG_LEVEL=debug LOG_FILE=$(CURDIR)/bin/protolog.txt protoc \
		...
		--go-plain_out=$(CURDIR) \
		--go-plain_opt=paths=source_relative,json_jx=true,jx_pb=true,pool=true,unified_oneof_json=true \
		--proto_path=$(CURDIR) \
		$(NDA_PROTO_FILES)
...
build-test-full: build .clean-test-full
	...
	LOG_LEVEL=debug LOG_FILE=$(CURDIR)/bin/protolog_full.txt protoc \
		...
		--go_opt=paths=source_relative \
		--go-plain_out=$(CURDIR) \
		--go-plain_opt=paths=source_relative,json_jx=true,jx_pb=true,pool=true \
		--proto_path=$(CURDIR) \
		$(FULL_PROTO_FILES)
