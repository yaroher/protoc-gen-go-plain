...
func (g *Generator) RenderJXJSON(typeIRs []*TypePbIR) error {
	for _, ir := range typeIRs {
		...
		outName := ir.File.GeneratedFilenamePrefix + "_plain.jx_json.go"
		...
		for _, name := range msgNames {
			msg := ir.Messages[name]
			g.renderJXJSONForMessage(out, ir, msg)
			out.P()
		}
	}
	return nil
...
func (g *Generator) renderJXJSONForMessage(out typeWriter, ir *TypePbIR, msg *typepb.Type) {
	...
	for _, field := range msg.Fields {
		...
		g.renderJXMarshalField(out, ir, field, fieldName, jsonName)
	}
	...
	for _, field := range msg.Fields {
		...
		jsonName := jsonTagFromPlain(g.plainName(field))
		out.P("\t\tcase \"", jsonName, "\":")
		g.renderJXUnmarshalField(out, ir, field, fieldName)
	}
...
func (g *Generator) renderJXMarshalField(out typeWriter, ir *TypePbIR, field *typepb.Field, fieldName, jsonName string) {
	...
	if cond != "" {
		...
		out.P("\t\t\treturn nil, jxEncodeError(\"", jsonName, "\")")
		out.P("\t\t}")
		g.renderJXEncodeValue(out, ir, field, fieldName, "\t\t")
		out.P("\t}")
		return
	}
...
func (g *Generator) renderJXEncodeValue(out typeWriter, ir *TypePbIR, field *typepb.Field, fieldName, indent string) {
	isRepeated := field.Cardinality == typepb.Field_CARDINALITY_REPEATED
	plainMessage := field.Kind == typepb.Field_TYPE_MESSAGE && g.isPlainMessage(ir, field.TypeUrl)
	isProtoMessage := field.Kind == typepb.Field_TYPE_MESSAGE && !plainMessage
	_, isOverride := g.overrideInfo(field)
...
func (g *Generator) renderJXUnmarshalField(out typeWriter, ir *TypePbIR, field *typepb.Field, fieldName string) {
	isRepeated := field.Cardinality == typepb.Field_CARDINALITY_REPEATED
	isPointer := g.isPointerField(field) || (field.Kind == typepb.Field_TYPE_MESSAGE && field.Cardinality != typepb.Field_CARDINALITY_REPEATED)
	plainMessage := field.Kind == typepb.Field_TYPE_MESSAGE && g.isPlainMessage(ir, field.TypeUrl)
	isProtoMessage := field.Kind == typepb.Field_TYPE_MESSAGE && !plainMessage
...
func (g *Generator) renderJXUnmarshalOneof(out typeWriter, ir *TypePbIR, msg *typepb.Type, oneof oneofField) {
	_ = ir
	msgGo := strcase.ToCamel(getShortName(msg.Name))
	out.P("\t\t\traw, err := d.Raw()")
	out.P("\t\t\tif err != nil {")
